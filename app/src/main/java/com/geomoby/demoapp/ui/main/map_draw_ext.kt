...
internal fun addPolygon(mMap: GoogleMap, fence: GeomobyFenceView) {
    fence.geometries?.let { geometries->
        for (geometry in geometries) {
            geometry?.points?.let { points->
                ...
                circleOptionsGeo.center(
                    LatLng(
                        geometry.centerPointY.toDouble(),
                        geometry.centerPointX.toDouble()
                    )
                )
                circleOptionsGeo.radius(geometry.fenceRadius.toDouble())
...
internal fun addCircle(mMap: GoogleMap, fence: GeomobyFenceView) {
    fence.geometries?.get(0)?.points?.get(0)?.let { point ->
...
internal fun addLine(mMap: GoogleMap, fence: GeomobyFenceView) {
    fence.geometries?.get(0)?.points?.let { points->
